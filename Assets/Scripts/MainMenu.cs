...
public class MainMenu : MonoBehaviour
{
    ...
    void CheckConnectedControllers()
    {
        ...
        if (keyboardUsed || mouseUsed)
        {
            ...
            DisplayPrompt("Use 'Space' to recharge gadget while next to a cow", rechargeText);
        }
        foreach (var device in InputSystem.devices)
        {
            if (device is Gamepad gamepad)
            {
                ...
                if (gamepad.name.Contains("Xbox")) // Xbox controller
                {
                    ...
                    DisplayPrompt("Use 'B' to recharge gadgets while next to a cow", rechargeText);
                }
                else if (gamepad.name.Contains("Sony")) // PS4 or PS5 controller
                {
                    ...
                    DisplayPrompt("Use 'Circle' to recharge gadgets while next to a cow", rechargeText);
                }
            }
        }
    }
